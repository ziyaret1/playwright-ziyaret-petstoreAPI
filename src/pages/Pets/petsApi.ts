...
export class PetsApi extends BaseAPI {
    ...
    async updatePetByForm(
        petId: number,
        body: PetFormUpdateDTO,
    ): Promise<ApiResponseDTO<PetFormUpdateResponseDTO>> {
        return this.post<PetDTO, PetFormUpdateResponseDTO>(
            `${petsEndpoints.POST_UPDATE_PET}${petId}${body}`,
        );
    }
}
